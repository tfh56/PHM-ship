...
一套在船上（或岸上服务器）运行的数据平台，把发动机和 GPS 的采集数据自动收集、存储、分析异常、报警，并通过大模型助理回答你的问题。
...
把 `PHM-ship` 文件夹拷机器上，比如放到 `/home/你的用户名/` 下。
...
用户名：`elastic`  
密码：安装完会显示在终端最后几行（也会保存在 `/data/elastic/.credentials` 文件里）
